  ...
  struct MemoryHeader {
    ...
    bool lock_extended_p() const {
      if(extended_header_p()) {
        ...
        return locked_count_field.get(header) == lock_extended();
      }
    }
    ...
    unsigned int lock_owner() const {
      if(extended_header_p()) {
        ...
        if(locked_count_field.get(header) > 0) {
    ...
    unsigned int lock_count() const {
      if(extended_header_p()) {
        ...
        return locked_count_field.get(header);
      }
    }
    ...
    void set_forwarded(MemoryHeader* address) {
      forwarded_field.set(header);
    }

    Object* forward() const {
      return reinterpret_cast<Object*>(header & ~0x7L);
    }
