...
namespace rubinius {
  ...
  RArray* MemoryHandle::get_rarray(STATE) {
    if(rarray_p()) {
      ...
      tuple->set_type_specific(Tuple::eRTuple);
      tuple->set_pinned();

      native_int size = array->tuple()->num_fields() - array->offset();

      RArray* rarray = new RArray;

      VALUE* ptr = reinterpret_cast<VALUE*>(tuple->field + array->offset());
      for(native_int i = array->offset(); i < size; i++) {
        ptr[i] = MemoryHandle::value(array->get(state, i));
      }
  ...
  void MemoryHandle::write_rarray(STATE) {
    if(rarray_p()) {
      ...
      if(tuple->type_specific() != Tuple::eRTuple) {
        tuple->set_type_specific(Tuple::eRTuple);
      }

      if(!tuple->pinned_p()) tuple->set_pinned();
